...
public class ThreeDimensionalChess : MonoBehaviour {
    ...
    */
    ...
    int ModuleId;
    private bool ModuleSolved, solveOnAnyPress;
    [SerializeField]
    bool DebugPuzzle;
    void Awake () {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (i == 4) {
                ...
                if (PB.CheckPiece(Pieces[6].T, Pieces[6].L, Pieces[6].R, Pieces[6].C, Test.L, Test.R, Test.C) && PB.CheckPiece(Test.T, Test.L, Test.R, Test.C, Pieces[0].L, Pieces[0].R, Pieces[0].C) && !CheckDuplicateCoordinatesForAll(Test)) {
                        Debug.LogFormat("[3D Chess #{0}] Submitting a {1} at {2}{3}{4} is valid.", ModuleId, Test.T, CoordinateNotation[0][Test.L], CoordinateNotation[1][Test.C], CoordinateNotation[2][Test.R]);
                        Solve();
                }
                else {
                    Strike();
                    ...
                    if (CheckDuplicateCoordinatesForAll(Test))
                    {
                        Debug.LogFormat("[3D Chess #{0}] The eighth piece overlaps with another piece on the board.", ModuleId);
                    }
                }
            }
            else {
...
*/
    ...
    void DebugGeneration () {
        Pieces[0].ChangePiece(0, 3, 1, PieceTypes[2]);
        Pieces[1].ChangePiece(0, 1, 3, PieceTypes[3]);
        Pieces[2].ChangePiece(0, 1, 1, PieceTypes[2]);
        Pieces[3].ChangePiece(0, 0, 0, PieceTypes[0]);
        Pieces[4].ChangePiece(0, 0, 1, PieceTypes[4]);
        Pieces[5].ChangePiece(3, 3, 1, PieceTypes[2]);
        Pieces[6].ChangePiece(3, 4, 0, PieceTypes[2]);
        ULTRATIMWI++;
        ...
        for (int i = 0; i < 125; i++) {
            Eighth.ChangePiece(i % 5, i / 5 % 5, i / 25 % 5, FinalPieceType);         //Generates the last piece such that it attacks the first and is attacked by the last. Probably.

            var overlappingCoordinate = false;
            for (int j = 0; j < 7 && !overlappingCoordinate; j++)
                overlappingCoordinate |= ShareExactCoordinate(Pieces[j], Eighth);
            if (overlappingCoordinate)
                continue;
    ...
    void GeneratePuzzle () {
        ...
        if (ULTRATIMWI > 20) {
            ...
            Debug.LogFormat("[3D Chess #{0}] More than 20 attempts taken to find a valid puzzle. Allowing solving on any press.", ModuleId, ULTRATIMWI);
            solveOnAnyPress = true;
        }
        ...
        var foundSolution = false;
        for (int i = 0; i < 125; i++) {
            Eighth.ChangePiece(i % 5, i / 5 % 5, i / 25 % 5, FinalPieceType);
            var overlappingCoordinate = false;
            for (int j = 0; j < 7 && !overlappingCoordinate; j++)
                overlappingCoordinate |= ShareExactCoordinate(Pieces[j], Eighth);
            if (overlappingCoordinate)
                continue;
            if (PB.CheckPiece(Eighth.T, Eighth.L, Eighth.R, Eighth.C, Pieces[0].L, Pieces[0].R, Pieces[0].C) &&
                PB.CheckPiece(Pieces[6].T, Pieces[6].L, Pieces[6].R, Pieces[6].C, Eighth.L, Eighth.R, Eighth.C)) {
                foundSolution = true;
                break;
            }
        }
        ...
        {  //Just in case.
            GeneratePuzzle();
            //Debug.LogFormat("[3D Chess #{0}] If this message showed up, something went wrong.", ModuleId, ULTRATIMWI);
            return;
        }
        Debug.LogFormat("[3D Chess #{0}] Generated a puzzle in {1} attempts.", ModuleId, ULTRATIMWI);
        LogPieces();
    ...
    bool ShareExactCoordinate (Piece P1, Piece P2) {
        return P1.R == P2.R && P1.C == P2.C && P1.L == P2.L;
        /*
        if (P1.R == P2.R) {
            if (P1.C == P2.C) {
            if (P1.L == P2.L) {
    ...
    void Solve () {
        ...
        ModuleSolved = true;
        var pickedWord = new[] { "GOOD", "DONE", "OKAY", "DEAF" }.PickRandom();
        for (var x = 0; x < SubmitDisplays.Length; x++)
            SubmitDisplays[x].text = pickedWord[x].ToString();
        if (LastPressed != -1)
        {
            StartCoroutine(KeyDepress(LastPressed));
            CoordinateDisplay.text = "!";
...
#pragma warning restore 414
    ...
    IEnumerator ProcessTwitchCommand (string Command) {
        if (Command.EqualsIgnoreCase("cycle"))
        {
            ...
            for (int i = 1; i < 7; i++)
            {
            yield return "trywaitcancel 2 Cycling buttons have been canceled!";
